...
import json
import sys
from wikimonscrape.digidb import DigiDB

G = nx.DiGraph()

db = DigiDB()
line = len(sys.argv) > 1 and sys.argv[1] or "Examon"
item = db.digimon_by_name_raw(line)
...
def recursive_add(G, item, seen: Set, do_next=False, do_previous=False):
    ...
    G.add_node(item[0], name=item[1], stage=stage, attribute=item[5].strip())
    seen.add(item[0])
    ...
    if do_previous:
        for prev_id in prev:
            prev_digimon = db.digimon_by_id_raw(prev_id)
    ...
    if do_next:
        for next_id in next_evo:
            next_digimon = db.digimon_by_id_raw(next_id)
